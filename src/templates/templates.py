from src.templates.template_registry import template



@template(
    name="prepare_work_environment",
    requires={"prepare", "work", "environment"},
)
...
@template(
    name="setup_video_player",
    params={"folder_name"}, 
    requires={"watch"},
    optionals={"anime", "movies"}
)
...
@template(name="open_copied_path", requires={"open", "copied", "path"})
        ...
        "task_graph": {
            ...
            "nodes": {
                "get_path": {
                    ...
                    "args": {"_as": "path"},
                    "on_success": "open_path",
                    "on_failure": "abort",
                },
                "open_path": {
                    "type": "action",
                    "controller": "open_folder",
                    "args": {"folder_path": "@folder", "select_file": "@file"},
                    "on_success": "done",
                    "on_failure": "abort"
                },
                "done": {"type": "noop", "message": "Opened path from clipboard"},
                "abort": {
                    "type": "abort",
                    "reason": "Copied path is not valid"
                }
            }
        }
    }
...
# @template(name="open_terminal_here", requires={"open", "terminal", ""})
